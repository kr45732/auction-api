...
async function startAuctionHouseLoop() {
    while (true) {
        ...
        sendWebhookInfoMessage(`Got ${oldPrices.length} auctions, inserting...`)

        collection.drop()
        let databaseStartTime = Date.now()
        collection.insertMany(oldPrices, async () => {
            sendWebhookInfoMessage(`Inserted ${oldPrices.length} auctions in ${Date.now() - databaseStartTime}ms\n`)
        })
...
app.get('/', async (req, res) => {
    ...
    try {
        ...
        return res.status(400).json({ error: 'Invalid query JSON provided.' })
    }

    if (typeof (query) != 'object') {
        return res.status(400).json({ error: 'Query JSON is an invalid data type' })
    }

    skyblockDB.collection(collectionName).find(query, { allowDiskUse: true }).toArray(async (err, found) => {
        if (err) {
